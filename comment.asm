...
LINHA_ATUAL_MISSIL:		; tabela que define a linha atual do míssil (começa na linhad a nave)
	WORD LINHA_NAVE		
...
TABELA_METEOROS:
	...
	WORD TIPO_METEORO_BOM, DEF_METEORO_1X1, 13, LINHA	
...
loop_teclados:							; Loop que chama o processo de teclado
	...
	CALL processo_mísseis				; Processo com lógica dos misseis
	;CALL processo_pausa
	;CALL processo_morte
	;CALL misseis
...
; * [Processo]
; *
; * processo_displays - Inicia e controla a variação da energia da nave apresentada nos displays.
; *
PROCESS SP_displays
processo_displays:
...
colisão_bom:
	...
	CALL apaga_meteoro
	MOV R5, TABELA_METEOROS					
	SUB R11, R5 						; Argumento para decisões_novo_meteoro
...
; *
PROCESS SP_míssil
processo_mísseis:						; chamado qd tecla missil premida (TECLA 1) 
inicio_processo_mísseis:
	MOV R0, [evento_disparar_míssil]
	MOV R3, 13							; Contador de movimentos do míssil (máximo é 12) - definido como 13 para comparar com 0
	MOV R0, -1
	MOV [evento_energia], R0			; Desbloqueia o evento de variar a energia (diminui)
inicialização_míssil:					
	MOV R0, 0 							
	MOV [TOCA_SOM], R0					; Som do disparo do míssil
	MOV R2, LINHA_NAVE
	MOV [LINHA_ATUAL_MISSIL], R2		; Inicializa a linha do míssil com a da nave (para ser desenhado na linha acima).
...
houve_colisão:
	...
	MOV R0, 0 							; Som colisão com um meteoro bom
	MOV [TOCA_SOM], R0
...
colisão_meteoro_mau:
	...
	MOV R0, 0							; som colisão com um meteoro mau
	MOV [TOCA_SOM], R0
	MOV R0, 1
